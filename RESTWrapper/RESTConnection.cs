    ...
    public class RESTConnection : IDisposable
    {
        ...
        internal async Task<T> InvokeMethod<T>(string name, BaseInput input)
        {
            ...
            if (mInitialized)
            {
                ...
                {
                    using (
                         HttpResponseMessage response = await mClient.PostAsJsonAsync(BasePath + '/' + name, input)
                         )

                    {
                        if (response.IsSuccessStatusCode)
                            ...
                            throw new WrongServerException("Server Is Not Properly Configed", new HttpRequestException());
                        else
                ...
                {
                    throw new WrongServerException("Server Is Not Properly Configed", ex);
                }
            }
            else
